...
public class TestAccuracyTAN {
    ...
    public static void main(String[] args) throws NeticaException {
        ...
        try
        {
            ...
            myNet = new Net(new Streamer("BBNs/BBNWithTAN_unedited.dne"));
		...
		{
			Streamer caseFile = new Streamer("CV5/"+ trainFile.get(i).getName());
			learnCPTs.LearnCPTs(myNet, caseFile, 3);
			myNet.compile();
			double error = getErrorRate(myNet,intensification,"CV5/" + testFiles.get(i).getName() );
            errorRate[i] = error;
		...
		System.out.println("**************From arrayList TAN****************");
		for (Double double1 : errorRateList) {
			System.out.printf("%.2f", double1);
			System.out.println();
		}
		
		/* System.out.println("The mean error from the five fold cross validation for TAN with count learning is :" + mean(errorRate));
...
*/
	...
	/*
	 * a static method to get errors from the bayesian network and testing data 
	 * 
	 */
