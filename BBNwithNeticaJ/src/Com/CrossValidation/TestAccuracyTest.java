...
public class TestAccuracyTest {
    ...
    public static void main(String[] args) throws NeticaException {
        ...
        {
            Streamer casefile = new Streamer("BBNwithNeticaJ/CV5/"+ trainFile.get(i).getName());
            learnCPTs.LearnCPTs(myNet, casefile, 2);
            myNet.compile();
            double error = getErrorRate (myNet, intensification,"BBNwithNeticaJ/CV5/" + testFiles.get(i).getName() );
            errorRate[i] = error;
            casefile.finalize();
            i++;
        }
